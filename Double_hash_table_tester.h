...
class Double_hash_table_tester:public Tester< Double_hash_table<Type> > {
	using Tester< Double_hash_table<Type> >::object;
	using Tester< Double_hash_table<Type> >::command;

	public:
		Double_hash_table_tester( Double_hash_table<Type> *obj = 0 ):Tester< Double_hash_table<Type> >( obj ) {
			// empty
		}
 ...
 * void process()
 *
 * Process the current command.  For double hash tables, these include:
 *
 *  Constructors
 *
...
void Double_hash_table_tester<Type>::process() {
	if ( command == "new" ) {
		object = new Double_hash_table<Type>();
		std::cout << "Okay" << std::endl;
	} else if ( command == "new:" ) {
		int n;
		std::cin >> n;
		object = new Double_hash_table<Type>( n );
		std::cout << "Okay" << std::endl;
